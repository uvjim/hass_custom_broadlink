...
from homeassistant.components.switch import PLATFORM_SCHEMA as SWITCH_PLATFORM_SCHEMA
...
async def async_setup_entry(
    ...
    elif device.api.type == "EHC31":
        switches.extend(BroadlinkEHCR31Slot(device, slot) for slot in range(1, 4))
        switches.extend(
            BroadlinkEHCR31Slot(device, slot, SlotType.CHILDLOCK)
...
class BroadlinkEHCR31Slot(BroadlinkSwitch):
    """Representation of a BG Electronics EHCR31 slot."""

    _attr_assumed_state = False
    _attr_has_entity_name = True

    def __init__(self, device, slot, slot_type: SlotType = SlotType.SOCKET):
        """Initialize the switch."""
        super().__init__(device, 1, 0)
        self._slot = slot
